...
const index_js_6 = require("../hash/index.js");
const index_js_7 = require("../signers/index.js");
const index_js_8 = require("../encoding/index.js");
 ...
 * @category Providers
 */
class JsonRpcSigner extends index_js_7.AbstractSigner {
    address;
    /**
    ...
    connect(provider) {
        (0, index_js_4.assert)(false, 'cannot reconnect JsonRpcSigner', 'UNSUPPORTED_OPERATION', {
    ...
    async sendUncheckedTransaction(_tx) {
        ...
        if ('from' in tx) {
            ...
            if (tx.from) {
                const _from = tx.from;
                promises.push((async () => {
                    const from = await (0, index_js_2.resolveAddress)(_from);
                    (0, index_js_4.assertArgument)(from != null && from.toLowerCase() === this.address.toLowerCase(), 'from address mismatch', 'transaction', _tx);
                    tx.from = from;
                })());
            }
            else {
            ...
            if (tx.to != null) {
                const _to = tx.to;
                promises.push((async () => {
                    tx.to = await (0, index_js_2.resolveAddress)(_to);
                })());
            }
        }
    ...
    async sendTransaction(tx) {
        ...
        const blockNumber = await this.provider.getBlockNumber((0, index_js_5.toShard)(zone));
        ...
        return await new Promise((resolve, reject) => {
            ...
            const checkTx = async () => {
                try {
                    ...
                    if ((0, index_js_4.isError)(error, 'CANCELLED') ||
                        (0, index_js_4.isError)(error, 'BAD_DATA') ||
                        (0, index_js_4.isError)(error, 'NETWORK_ERROR') ||
                        (0, index_js_4.isError)(error, 'UNSUPPORTED_OPERATION')) {
                        if (error.info == null) {
                            error.info = {};
                        }
                    ...
                    if ((0, index_js_4.isError)(error, 'INVALID_ARGUMENT')) {
                        invalids++;
                        if (error.info == null) {
                            error.info = {};
                        }
                    ...
                    this.provider.emit('error', (0, provider_js_1.zoneFromHash)(hash), (0, index_js_4.makeError)('failed to fetch transation after sending (will try again)', 'UNKNOWN_ERROR', {
                        error,
                    }));
                }
    ...
    async signTransaction(_tx) {
        ...
        if ('from' in tx) {
            if (tx.from) {
                const from = await (0, index_js_2.resolveAddress)(tx.from);
                (0, index_js_4.assertArgument)(from != null && from.toLowerCase() === this.address.toLowerCase(), 'from address mismatch', 'transaction', _tx);
                tx.from = from;
            }
            else {
    ...
    async signMessage(_message) {
        const message = typeof _message === 'string' ? (0, index_js_8.toUtf8Bytes)(_message) : _message;
        return await this.provider.send('personal_sign', [(0, index_js_4.hexlify)(message), this.address.toLowerCase()]);
    }
    /**
     * Signs typed data.
     *
    ...
    async signTypedData(domain, types, _value) {
        ...
        return await this.provider.send('quai_signTypedData_v4', [
            ...
            JSON.stringify(index_js_6.TypedDataEncoder.getPayload(domain, types, value)),
        ]);
    }
    /**
     * Unlocks the account.
    ...
    async _legacySignMessage(_message) {
        const message = typeof _message === 'string' ? (0, index_js_8.toUtf8Bytes)(_message) : _message;
        return await this.provider.send('quai_sign', [this.address.toLowerCase(), (0, index_js_4.hexlify)(message)]);
    }
}
...
class JsonRpcApiProvider extends abstract_provider_js_1.AbstractProvider {
            ...
            while (payloads.length) {
                ...
                (async () => {
                    ...
                    const processPayloads = async (key, value, now) => {
                        ...
                        const shard = key ? (0, index_js_5.toShard)(key) : index_js_5.Shard.Prime;
                        const zone = shard.length < 4 ? undefined : (0, index_js_5.toZone)(shard);
                    ...
                    try {
                        ...
                        for (const { resolve, reject, payload, shard } of batch) {
                            if (this.destroyed) {
                                reject((0, index_js_4.makeError)('provider destroyed; cancelled request', 'UNSUPPORTED_OPERATION', {
                                    operation: payload.method,
                                }));
                                continue;
                            }
                            if (shard) {
                                lastZone = shard.length < 4 ? undefined : (0, index_js_5.toZone)(shard);
                            }
                            else {
                                lastZone = undefined;
                            }
                            ...
                            if (resp == null) {
                                const error = (0, index_js_4.makeError)('missing response for request', 'BAD_DATA', {
                                    value: result,
                                    info: { payload },
                                });
    ...
    constructor(network, options) {
        ...
        if (typeof staticNetwork === 'boolean') {
            (0, index_js_4.assertArgument)(!staticNetwork || network !== 'any', "staticNetwork cannot be used on special network 'any'", 'options', options);
            ...
            (0, index_js_4.assertArgument)(network == null || staticNetwork.matches(network), 'staticNetwork MUST match network object', 'options', options);
     ...
     */
    get _network() {
        (0, index_js_4.assert)(this.#network, 'network is not available yet', 'NETWORK_ERROR');
        return this.#network;
    }
    /**
    ...
    async _perform(req) {
        ...
        if (request != null) {
            const shard = 'shard' in req ? req.shard : 'zone' in req ? (0, index_js_5.toShard)(req.zone) : undefined;
            if (req.method === 'getRunningLocations') {
    ...
    async _detectNetwork() {
        ...
        if (this.ready) {
            this.#pendingDetectNetwork = (async () => {
                try {
                    const result = network_js_1.Network.from((0, index_js_4.getBigInt)(await this.send('quai_chainId', [])));
                    this.#pendingDetectNetwork = null;
                    return result;
                }
                catch (error) {
        ...
        (async () => {
            ...
            while (this.#network == null && !this.destroyed && retries < maxRetries) {
                try {
                    ...
                    this.emit('error', undefined, (0, index_js_4.makeError)('failed to bootstrap network detection', 'NETWORK_ERROR', {
            ...
            if (retries >= maxRetries) {
                ...
                (0, index_js_4.makeError)('failed to bootstrap network detection', 'NETWORK_ERROR', {
                    event: 'initial-network-discovery',
                    info: { retries },
                });
            }
    ...
    _getSubscriber(sub) {
        ...
        if (sub.type === 'pending') {
            return new subscriber_filterid_js_1.FilterIdPendingSubscriber(this, sub.zone);
        }
        if (sub.type === 'event') {
        ...
        if ('from' in tx || ('to' in tx && 'data' in tx)) {
            ...
            if (tx.txInputs != null) {
                result['txInputs'] = tx.txInputs.map((input) => ({
                    txhash: (0, index_js_4.hexlify)(input.txhash),
                    index: (0, index_js_4.toQuantity)((0, index_js_4.getBigInt)(input.index, `tx.txInputs.${input.index}`)),
                    pubkey: (0, index_js_4.hexlify)(input.pubkey),
                }));
            }
            if (tx.txOutputs != null) {
                result['txOutputs'] = tx.txOutputs.map((output) => ({
                    address: (0, index_js_4.hexlify)(output.address),
                    denomination: (0, index_js_4.toQuantity)((0, index_js_4.getBigInt)(output.denomination, `tx.txOutputs.${output.denomination}`)),
                }));
            }
        }
        return result;
    ...
    getRpcError(payload, _error, shard) {
        ...
        if (method === 'quai_estimateGas' && error.message) {
            ...
            if (!msg.match(/revert/i) && msg.match(/insufficient funds/i)) {
                return (0, index_js_4.makeError)('insufficient funds', 'INSUFFICIENT_FUNDS', {
        ...
        if (method === 'quai_getTransactionByHash' && error.message && error.message.match(/transaction not found/i)) {
            return (0, index_js_4.makeError)('transaction not found', 'TRANSACTION_NOT_FOUND', { info: { payload, error, shard } });
        }
        if (typeof error.message === 'string' && error.message.match(/user denied|quais-user-denied/i)) {
            ...
            return (0, index_js_4.makeError)(`user rejected action`, 'ACTION_REJECTED', {
        ...
        if (method === 'quai_sendRawTransaction' || method === 'quai_sendTransaction') {
            ...
            if (message.match(/nonce/i) && message.match(/too low/i)) {
                return (0, index_js_4.makeError)('nonce has already been used', 'NONCE_EXPIRED', {
                    transaction,
                    info: { error, shard },
                });
            }
            ...
            if (message.match(/replacement transaction/i) && message.match(/underpriced/i)) {
                return (0, index_js_4.makeError)('replacement fee too low', 'REPLACEMENT_UNDERPRICED', {
                    transaction,
                    info: { error, shard },
                });
            }
            ...
            if (message.match(/already known/i)) {
                return (0, index_js_4.makeError)('transaction already known', 'TRANSACTION_ALREADY_KNOWN', { info: { error, shard } });
            }
        }
        ...
        if (unsupported) {
            return (0, index_js_4.makeError)('unsupported operation', 'UNSUPPORTED_OPERATION', {
                operation: payload.method,
        ...
        if (message.match('Provider failed to initialize on creation. Run initialize or create a new provider.')) {
            return (0, index_js_4.makeError)('Provider failed to initialize on creation. Run initUrlMap or create a new provider.', 'PROVIDER_FAILED_TO_INITIALIZE', {
                info: { payload, error, shard },
            });
        }
        return (0, index_js_4.makeError)('could not coalesce error', 'UNKNOWN_ERROR', { error, payload, shard });
    }
    /**
    ...
    send(method, params, shard, now) {
        const continueSend = () => {
            if (this.destroyed) {
                return Promise.reject((0, index_js_4.makeError)('provider destroyed; cancelled request', 'UNSUPPORTED_OPERATION', { operation: method }));
            }
...
class JsonRpcProvider extends JsonRpcApiProvider {
    ...
    _getConnection(shard) {
        ...
        return new index_js_4.FetchRequest(connection.url);
    }
...
function spelunkData(value) {
    ...
    if (typeof value.message === 'string' && value.message.match(/revert/i) && (0, index_js_4.isHexString)(value.data)) {

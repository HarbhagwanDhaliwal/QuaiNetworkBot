...
const shards_js_1 = require("../utils/shards.js");
const shards_js_2 = require("../constants/shards.js");
/**
...
async function getSubscription(_event, zone) {
    ...
    if ((0, index_js_3.isHexString)(_event, 32)) {
        const eventBytes = (0, index_js_3.getBytes)(_event);
        const ninthBit = (eventBytes[1] & 0x80) === 0x80;
        const hash = _event.toLowerCase();
        zone = (0, index_js_2.toZone)(hash.slice(0, 4));
        if (ninthBit) {
    ...
    if (_event.type && _event.address) {
        const address = (0, index_js_1.formatMixedCaseChecksumAddress)((0, index_js_3.isHexString)(_event.address) ? _event.address : await (0, index_js_1.resolveAddress)(_event.address));
        const filter = {
            type: _event.type,
            address: address,
        };
        if (!zone) {
            zone = (0, index_js_2.toZone)(address.slice(0, 4));
        }
        ...
        if (event.address) {
            ...
            const addAddress = (addr) => {
                if ((0, index_js_3.isHexString)(addr)) {
                    addresses.push((0, index_js_1.formatMixedCaseChecksumAddress)(addr));
                }
                else {
                    promises.push((async () => {
                        addresses.push((0, index_js_1.formatMixedCaseChecksumAddress)(await (0, index_js_1.resolveAddress)(addr)));
                    })());
                }
            };
            ...
            if (!zone) {
                zone = (0, index_js_2.toZone)(addresses[0].slice(0, 4));
            }
            filter.address = concisify(addresses.map((a) => a.toLowerCase()));
            if (!filter.nodeLocation) {
                filter.nodeLocation = (0, shards_js_1.getNodeLocationFromZone)(zone);
            }
        }
        else {
            if (!zone) {
...
class AbstractProvider {
    ...
    async initialize(urls) {
        ...
        try {
            const primeSuffix = this.#options.usePathing ? `/${(0, shards_js_2.fromShard)(index_js_2.Shard.Prime, 'nickname')}` : ':9001';
            if (urls instanceof index_js_3.FetchRequest) {
                urls.url = urls.url.split(':')[0] + ':' + urls.url.split(':')[1] + primeSuffix;
                this._urlMap.set(index_js_2.Shard.Prime, urls);
                this.#connect.push(urls);
                const shards = await this._waitGetRunningLocations(index_js_2.Shard.Prime, true);
                shards.forEach((shard) => {
                    const port = 9200 + 20 * shard[0] + shard[1];
                    const shardEnum = (0, index_js_2.toShard)(`0x${shard[0].toString(16)}${shard[1].toString(16)}`);
                    const shardSuffix = this.#options.usePathing ? `/${(0, shards_js_2.fromShard)(shardEnum, 'nickname')}` : `:${port}`;
                    this._urlMap.set(shardEnum, new index_js_3.FetchRequest(urls.url.split(':')[0] + ':' + urls.url.split(':')[1] + shardSuffix));
                });
                return;
            }
            if (Array.isArray(urls)) {
                for (const url of urls) {
                    ...
                    const primeConnect = new index_js_3.FetchRequest(primeUrl);
                    this._urlMap.set(index_js_2.Shard.Prime, primeConnect);
                    ...
                    shards.forEach((shard) => {
                        ...
                        const shardSuffix = this.#options.usePathing
                            ? `/${(0, shards_js_2.fromShard)(shardEnum, 'nickname')}`
                            : `:${port}`;
                        this._urlMap.set((0, index_js_2.toShard)(`0x${shard[0].toString(16)}${shard[1].toString(16)}`), new index_js_3.FetchRequest(url.split(':')[0] + ':' + url.split(':')[1] + shardSuffix));
                    });
                }
            }
    ...
    async zoneFromAddress(_address) {
        ...
        return (0, index_js_2.toZone)((await address).slice(0, 4));
    }
    /**
     ...
     */
    shardFromHash(hash) {
        return (0, index_js_2.toShard)(hash.slice(0, 4));
    }
    /**
     ...
     */
    zoneFromHash(hash) {
        return (0, index_js_2.toZone)(hash.slice(0, 4));
    }
    /**
    ...
    async getLatestQuaiRate(zone, amt) {
        const blockNumber = await this.getBlockNumber((0, index_js_2.toShard)(zone));
    ...
    async getQuaiRateAtBlock(zone, blockTag, amt) {
        let resolvedBlockTag = this._getBlockTag((0, index_js_2.toShard)(zone), blockTag);
        ...
        return (0, index_js_3.getBigInt)(await this.#perform({
            ...
            amt: (0, index_js_3.toQuantity)(String(amt)),
            zone: zone,
        }));
    }
    /**
    ...
    async getProtocolExpansionNumber() {
        return (0, index_js_3.getNumber)(await this.#perform({ method: 'getProtocolExpansionNumber' }));
    }
    /**
    ...
    async getActiveRegions() {
        ...
        const shards = [index_js_2.Shard.Cyprus];
        if (protocolExpansionNumber >= 1) {
            shards.push(index_js_2.Shard.Paxos);
        }
        if (protocolExpansionNumber >= 3) {
            shards.push(index_js_2.Shard.Hydra);
        }
    ...
    async getActiveZones() {
        const protocolExpansionNumber = await this.getProtocolExpansionNumber();
        const zones = [index_js_2.Zone.Cyprus1];
        if (protocolExpansionNumber >= 1) {
            zones.push(index_js_2.Zone.Cyprus2);
        }
        if (protocolExpansionNumber >= 2) {
            zones.push(index_js_2.Zone.Paxos1, index_js_2.Zone.Paxos2);
        }
        if (protocolExpansionNumber >= 3) {
            zones.push(index_js_2.Zone.Cyprus3, index_js_2.Zone.Paxos3, index_js_2.Zone.Hydra1, index_js_2.Zone.Hydra2, index_js_2.Zone.Hydra3);
        }
    ...
    _wrapTransactionReceipt(value, network) {
        const formattedReceipt = (0, format_js_1.formatTransactionReceipt)(value);
        return new provider_js_1.TransactionReceipt(formattedReceipt, this);
    }
    /**
    ...
    _wrapTransactionResponse(tx, network) {
        try {
            const formattedTx = (0, format_js_1.formatTransactionResponse)(tx);
            if (tx.type === '0x0' || tx.type === 0) {
                return new provider_js_1.QuaiTransactionResponse(formattedTx, this);
            }
            else if (tx.type === '0x1' || tx.type === 1) {
                return new provider_js_1.ExternalTransactionResponse(formattedTx, this);
            }
            else if (tx.type === '0x2' || tx.type === 2) {
                return new provider_js_1.QiTransactionResponse(formattedTx, this);
            }
            else {
    ...
    async getBlockNumber(shard) {
        const blockNumber = (0, index_js_3.getNumber)(await this.#perform({ method: 'getBlockNumber', shard: shard }), '%response');
    ...
    _getBlockTag(shard, blockTag) {
        ...
        if (typeof blockTag === 'bigint') {
            blockTag = (0, index_js_3.getNumber)(blockTag, 'blockTag');
        }
        if (typeof blockTag === 'number') {
            ...
            return this.getBlockNumber(shard).then((b) => (0, index_js_3.toQuantity)(b + blockTag));
        }
    ...
    _getFilter(filter) {
        ...
        const zone = (0, shards_js_1.getZoneFromNodeLocation)(filter.nodeLocation);
        let fromBlock = undefined;
        if ('fromBlock' in filter) {
            fromBlock = this._getBlockTag((0, index_js_2.toShard)(zone), filter.fromBlock);
        }
        let toBlock = undefined;
        if ('toBlock' in filter) {
            toBlock = this._getBlockTag((0, index_js_2.toShard)(zone), filter.toBlock);
        }
    ...
    _getTransactionRequest(_request) {
        ...
        ['to', 'from', 'inputs', 'outputs'].forEach((key) => {
            ...
            const addr = Array.isArray(request[key])
                ? 'address' in request[key][0]
                    ...
                    : request[key].map((it) => (0, index_js_1.computeAddress)(it.pubkey))
                : (0, index_js_1.resolveAddress)(request[key]);
            if (isPromise(addr)) {
        ...
        if (request.blockTag != null) {
            const getBlockTag = async () => {
                const zone = await this.zoneFromAddress((0, provider_js_1.addressFromTransactionRequest)(_request));
                const shard = (0, index_js_2.toShard)(zone);
    ...
    async getFeeData(zone, txType = true) {
        const getFeeDataFunc = async () => {
            const { gasPrice, minerTip } = await (0, index_js_3.resolveProperties)({
                gasPrice: (async () => {
                    try {
                        const value = await this.#perform({ method: 'getGasPrice', txType, zone: zone });
                        return (0, index_js_3.getBigInt)(value, '%response');
                    }
                    catch (error) {
                        console.log(error);
                    }
            ...
            return new provider_js_1.FeeData(gasPrice, baseMinerTip);
        };
    ...
    async estimateFeeForQi(_tx) {
        const zone = await this.zoneFromAddress((0, provider_js_1.addressFromTransactionRequest)(_tx));
        return (0, index_js_3.getBigInt)(await this.#perform({
    ...
    async #call(tx, blockTag, attempt, zone) {
        ...
        return (0, index_js_3.hexlify)(await this._perform({ method: 'call', transaction, blockTag, zone }));
    }
    ...
    async getLockedBalance(address) {
        return (0, index_js_3.getBigInt)(await this.#getAccountValue({ method: 'getLockedBalance' }, address), '%response');
    }
    async getOutpointsByAddress(address) {
        return (0, format_js_1.formatOutpoints)(await this.#getAccountValue({ method: 'getOutpointsByAddress' }, address, 'latest'));
    }
    ...
    async broadcastTransaction(zone, signedTx) {
        const type = (0, index_js_4.decodeProtoTransaction)((0, index_js_3.getBytes)(signedTx)).type;
        try {
            const { blockNumber, hash, network } = await (0, index_js_3.resolveProperties)({
                blockNumber: this.getBlockNumber((0, index_js_2.toShard)(zone)),
                hash: this._perform({
            ...
            const tx = type == 2 ? index_js_5.QiTransaction.from(signedTx) : index_js_5.QuaiTransaction.from(signedTx);
            const txObj = tx.toJSON();
            this.#validateTransactionHash(tx.hash || '', hash);
            if (type == 2) {
                return new provider_js_1.QiTransactionResponse(txObj, this);
            }
    ...
    async getTransaction(hash) {
        const zone = (0, index_js_2.toZone)(this.shardFromHash(hash));
        const { network, params } = await (0, index_js_3.resolveProperties)({
            network: this.getNetwork(),
    ...
    async getOutpointDeltas(addresses, startHash, endHash) {
        ...
        for (const addr of addresses) {
            (0, index_js_3.assertArgument)((0, index_js_1.isQiAddress)(addr), `Invalid Qi address: ${addr}`, 'addresses', addresses);
        }
        // Validate block hashes
        (0, index_js_3.assertArgument)((0, index_js_3.isHexString)(startHash, 32), 'invalid startHash', 'startHash', startHash);
        if (endHash) {
            (0, index_js_3.assertArgument)((0, index_js_3.isHexString)(endHash, 32), 'invalid endHash', 'endHash', endHash);
        }
        else {
            endHash = 'latest';
        }
        ...
        const startBlockNumber = (0, index_js_3.getNumber)(startBlock.woHeader.number, 'startBlockNumber');
        const endBlockNumber = (0, index_js_3.getNumber)(endBlock.woHeader.number, 'endBlockNumber');
        (0, index_js_3.assertArgument)(startBlockNumber <= endBlockNumber, 'startBlockNumber must be less than or equal to endBlockNumber', 'startBlockNumber', startBlockNumber);
        ...
        const promises = ranges.map((range) => {
            return this.#perform({
                ...
                zone: zone,
            }).then(format_js_1.formatOutpointDeltas);
        });
    ...
    async getLogs(_filter) {
        ...
        const { network, params } = await (0, index_js_3.resolveProperties)({
            ...
            params: this.#perform({
                method: 'getLogs',
                filter,
                zone: (0, shards_js_1.getZoneFromNodeLocation)(filter.nodeLocation),
            }),
        });
    ...
    async waitForTransaction(hash, _confirms, timeout) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            listener(await this.getBlockNumber((0, index_js_2.toShard)(zone)));
        });
    }
    // TODO: not implemented yet
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    async waitForBlock(shard, blockTag) {
        (0, index_js_3.assert)(false, 'not implemented yet', 'NOT_IMPLEMENTED', {
    ...
    _getSubscriber(sub) {
        switch (sub.type) {
            ...
            case 'block': {
                const subscriber = new subscriber_polling_js_1.PollingBlockSubscriber(this, sub.zone);
            ...
            case 'transaction':
                return new subscriber_polling_js_1.PollingTransactionSubscriber(this, sub.hash, sub.zone);
            case 'qiTransaction':
                return new subscriber_polling_js_1.PollingQiTransactionSubscriber(this, sub.hash, sub.zone);
            case 'orphan':
                return new subscriber_polling_js_1.PollingOrphanSubscriber(this, sub.filter, sub.zone);
        }

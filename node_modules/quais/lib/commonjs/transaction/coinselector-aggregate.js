...
exports.AggregateCoinSelector = void 0;
 ...
 */
class AggregateCoinSelector extends abstract_coinselector_js_1.AbstractCoinSelector {
    /**
    ...
    performSelection(config) {
        ...
        this.totalInputValue = eligibleUTXOs.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
        ...
        const totalInputValueAboveMaxDenomination = bigDenominationUTXOs.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
        ...
        const feeInputsValue = feeInputs.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
    ...
    createOptimalDenominations(value) {
        const outputs = [];
        let remaining = value;
        for (let i = utxo_js_1.denominations.length - 1; i >= 0 && remaining > BigInt(0); i--) {
            const denomination = utxo_js_1.denominations[i];
            while (remaining >= denomination) {
                const output = new utxo_js_1.UTXO();
    ...
    getInputsForFee(inputsToAggregate, eligibleUTXOs, fee) {
        ...
        for (const utxo of sortedUTXOs) {
            sum += BigInt(utxo_js_1.denominations[utxo.denomination]);
            feeInputs.push(utxo);
            if (sum >= fee) {
                return feeInputs;
            }
    ...
    getInputsToAggregate(smallDenominationsUTXOs, valueToAggregate) {
        ...
        for (const utxo of sortedUTXOs) {
            ...
            if (inputsToAggregate.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0)) ===
                valueToAggregate) {
...
exports.AggregateCoinSelector = AggregateCoinSelector;
//# sourceMappingURL=coinselector-aggregate.js.map
...
exports.FewestCoinSelector = void 0;
// import { bigIntAbs } from '../utils/maths.js';
const abstract_coinselector_js_1 = require("./abstract-coinselector.js");
 ...
 */
class FewestCoinSelector extends abstract_coinselector_js_1.AbstractCoinSelector {
    /**
    ...
    performSelection(config) {
        ...
        const singleUTXO = sortedUTXOs.find((utxo) => BigInt(utxo_js_1.denominations[utxo.denomination]) >= totalRequired);
        if (singleUTXO) {
            // Use the smallest UTXO that can cover the total required amount
            this.selectedUTXOs.push(singleUTXO);
            this.totalInputValue = BigInt(utxo_js_1.denominations[singleUTXO.denomination]);
        }
        else {
            ...
            this.totalInputValue = this.selectedUTXOs.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
        }
        // Create outputs
    ...
    findMinimalUTXOSet(sortedUTXOs, totalRequired) {
        ...
        for (const utxo of utxos) {
            ...
            totalValue += BigInt(utxo_js_1.denominations[utxo.denomination]);
        }
        if (totalValue >= totalRequired) {
    ...
    createSpendOutputs(amount) {
        ...
        const spendDenominations = (0, utxo_js_1.denominate)(amount, maxInputDenomination);
    ...
    createChangeOutputs(change) {
        ...
        const changeDenominations = (0, utxo_js_1.denominate)(change, maxInputDenomination);
    ...
    calculateTotalOutputValue() {
        const spendValue = this.spendOutputs.reduce((sum, output) => sum + BigInt(utxo_js_1.denominations[output.denomination]), BigInt(0));
        const changeValue = this.changeOutputs.reduce((sum, output) => sum + BigInt(utxo_js_1.denominations[output.denomination]), BigInt(0));
    ...
    getMaxDenomination(utxos) {
        return utxos.reduce((max, utxo) => {
            const denomValue = BigInt(utxo_js_1.denominations[utxo.denomination]);
    ...
    increaseFee(additionalFeeNeeded) {
        ...
        const totalChange = this.changeOutputs.reduce((sum, output) => BigInt(sum) + BigInt(utxo_js_1.denominations[output.denomination]), BigInt(0));
        ...
        for (const utxo of sortedUTXOs) {
            ...
            this.totalInputValue += BigInt(utxo_js_1.denominations[utxo.denomination]);
            remainingFee -= BigInt(utxo_js_1.denominations[utxo.denomination]);
    ...
    decreaseFee(feeReduction) {
        ...
        for (const input of sortedInputs) {
            const inputValue = BigInt(utxo_js_1.denominations[input.denomination]);
